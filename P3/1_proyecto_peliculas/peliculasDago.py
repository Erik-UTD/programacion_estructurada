...
def modificarPeliculas():
    ...
    if conexionBD!=None:
        ...
        cursor=conexionBD.cursor()
        id=input("dame el ID de la pelicula a modificar: ").strip()
        sql="select * from peliculas where id=%s"
        val=(id,)
        cursor.execute(sql,val)
        registros=cursor.fetchall()
...
def borrarCaracteristica():
    ...
    if conexionBD!=None:
        ...
        registros=cursor.fetchall()
        if registros:
            resp=input(f"Deseas borrar la pelicula {id}? (Si/No): ").lower().strip()  
            if resp=="si":
                sql="delete from peliculas where id=%s"
                cursor.execute(sql,)
                conexionBD.commit()
